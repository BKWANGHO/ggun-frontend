...
export default function ArticleDetail({ params }: { params: { id: string } }) {
  ...
  const fetchData = async (): Promise<IArticle> => {
    const response = await fetchArticleDatail(params.id);
  ...
  const { data: article } = useQuery<IArticle>(
      {
          queryKey: ["artiDetail", params.id],
          queryFn: fetchData,
      }
  );

  // const article = articleDummy[parseInt(params.id)]
      ...
      <div className='w-[70%]'>
        ...
        <WhiteBox style="flex justify-center items-center">
          <div className="w-[80%]" >
            ...
            <div className="text-center text-[36px] my-3">{article&&article.title} <hr /></div>

            <div className='flex py-2 w-full border-b-2'>
              <div className='w-1/2'>
                작성자 : {article&&article.writerId} 부서 {article&&article.writerId} 님
              </div>
              <div className='text-right w-1/2'>작성일자 : {article&&article.regDate} </div>
            </div>
            ...
            <div className="icons flex text-gray-500 gap-3 cursor-point">
              ...
              <div className="count ml-auto text-gray-400 text-xs font-semibold">{article&&article.content?.length}/300</div>
            </div>
